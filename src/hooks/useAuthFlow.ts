...
/**
 * Шаги аутентификации:
 *  - 'loading'        — первичная загрузка (initAuth), пока проверяем токен / ждём 3.5 секунды
 *  - 'language'       — выбор языка
 *  - 'invite_code'    — ввод реферального кода
 *  - 'skin'           — выбор скина
 *  - 'final_loading'  — короткий финальный лоадер (если нужно)
 *  - 'completed'      — всё готово, рендерим children
 */
...
export const useAuthFlow = () => {
  ...
  const { isError: isUsersError } = useGetUserQuery(); // Для проверки access token
  ...
  /**
   * Проверяем, есть ли у нас access_token, и не вернул ли бэкенд ошибку при запросе getUser
   */
  const isTokenValid = () => {
  ...
  const handleInviteCodeContinue = async () => {
    // Переход к этапу авторизации после ввода кода
    const success = await performAuthentication();
    if (success) {
      saveCurrentStep('skin');
    } else {
